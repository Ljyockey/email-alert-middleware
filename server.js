...
const emailAlertFunction = (err, req, res, next) => {
    logger.info('emailAlertFunciton activated');
  if (err instanceof BarError || err instanceof FooError) {
    let emailData = 
      {
       from: "yockey.alerts@gmail.com",
       to: "linyockey@gmail.com",
       subject: `${err.name} found in cold-nation`,
       text: `${err.name} found in cold-nation. \n Detals: ${err.stack}`
      };
    sendEmail(emailData);
  }
  next();
